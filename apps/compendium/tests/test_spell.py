...
class SpellAPITests(APITestCase):
    def setUp(self):
        self.source = Source.objects.create(code="PHB", name="Player's Handbook")
        self.alt_source = Source.objects.create(code="XGE", name="Xanathar's Guide")
        ...
        self.detail_url = reverse("spells-detail", kwargs={"pk": self.spell.pk})
    ...
    def test_anonymous_user_cannot_create_spell(self):
        response = self.client.post(self.list_url, self.default_payload(), format="json")
    ...
    def test_moderator_can_create_spell(self):
        ...
        self.assertEqual(list(created_spell.classes.values_list("id", flat=True)), [self.cleric.id])
    ...
    def test_create_spell_rejects_time_casting_without_required_fields(self):
        ...
        self.assertEqual(response.data["casting_value"][0], "Required when casting_type is TIME")
        self.assertEqual(response.data["casting_unit"][0], "Required when casting_type is TIME")
    ...
    def test_create_spell_rejects_distance_range_without_required_fields(self):
        ...
        self.assertEqual(response.status_code, 400)
        self.assertEqual(response.data["range_value"][0], "Required when range_type is DISTANCE")
        self.assertEqual(response.data["range_unit"][0], "Required when range_type is DISTANCE")
    ...
    def test_create_spell_rejects_area_without_value_and_unit(self):
        ...
        self.assertEqual(response.status_code, 400)
        self.assertEqual(response.data["range_area_value"][0], "Required when range_area_type is set")
        self.assertEqual(response.data["range_area_unit"][0], "Required when range_area_type is set")
    ...
    def test_create_spell_rejects_concentration_for_instant_duration(self):
        ...
        self.assertEqual(response.status_code, 400)
        self.assertEqual(response.data["concentration"][0], "Instant spells cannot require concentration")
    ...
    def test_partial_update_rejects_switch_to_time_duration_without_required_fields(self):
        self.authenticate(self.moderator)
        ...
        self.assertEqual(response.status_code, 400)
        self.assertEqual(response.data["duration_value"][0], "Required when duration_type is TIME")
        self.assertEqual(response.data["duration_unit"][0], "Required when duration_type is TIME")